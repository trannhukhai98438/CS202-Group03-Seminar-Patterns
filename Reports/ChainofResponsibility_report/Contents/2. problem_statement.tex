...
\begin{lstlisting}[language=C++, caption={Naïve request processing using nested conditionals}]
struct Request {
...
private:
    bool authenticate(const Request& r) { return r.isAuthenticated; }
    bool authorize(const Request& r) { return r.isAuthorized; }
    bool validate(const Request& r) { return r.isValid; }
};
\end{lstlisting}
...
\begin{itemize}
    \item \textbf{Violation of the Open/Closed Principle (OCP) \cite{martin2000design}:} Introducing new processing steps (e.g., data decryption or auditing) requires direct modification of the \texttt{RequestHandler} class, increasing the risk of unintended side effects.
