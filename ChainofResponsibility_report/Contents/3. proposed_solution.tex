...
To overcome the limitations of high coupling, rigid control flow, and poor maintainability inherent in the naïve conditional approach, we propose restructuring the system using the \textbf{Chain of Responsibility (CoR)} behavioral design pattern. This pattern introduces a more modular and extensible mechanism for processing requests through a sequence of independent components.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{UML Class Diagram for Chain of Responsibility Pattern}
    \label{fig:cor_class_diagram}
\end{figure}

\subsubsection{Refactored Implementation}
The following Java implementation demonstrates how the Chain of Responsibility pattern replaces monolithic conditional logic with a flexible, object-oriented structure:

\begin{lstlisting}[language=Java, caption={Chain of Responsibility Pattern Implementation}]
...
public abstract class BaseHandler {
    ...
    public BaseHandler setNext(BaseHandler nextHandler) {
        ...
        return nextHandler; // Enables fluent chaining
    }
...
public class AuthenticationHandler extends BaseHandler {
    ...
    public void handle(Request request) {
        if (!request.isAuthenticated()) {
            System.out.println("[FAIL] Authentication failed. Terminating chain.");
            return;
        }
...
public class ValidationHandler extends BaseHandler {
    ...
    public void handle(Request request) {
        if (!request.isValid()) {
            System.out.println("[FAIL] Data validation failed. Terminating chain.");
            return;
        }
        System.out.println("[SUCCESS] Data validation passed.");
        super.handle(request);
    }
}
...
public class Application {
    public static void main(String[] args) {
        BaseHandler authHandler = new AuthenticationHandler();
        BaseHandler validHandler = new ValidationHandler();

        authHandler.setNext(validHandler);

        Request userRequest = new Request(true, true);
...
\begin{figure}[H]
    ...
    \node (auth) at (4.0,0) [entity] {:AuthHandler};
    \node (val) at (8.0,0) [entity] {:ValidationHandler};

    % Draw Lifelines
    \draw [lifeline] (client) -- (0,-5.2);
    \draw [lifeline] (auth) -- (4.0,-5.2);
    \draw [lifeline] (val) -- (8.0,-5.2);

    % Messages and Execution Flow
    % 1. Client calls AuthHandler
    \draw [msg] (0,-0.9) -- node[above, font=\xsf] {handle(request)} (4.0,-0.9);
    \node[anchor=west, font=\tiny, color=blue!80!black] at (4.1, -1.3) {[Authentication Passed]};
    
    % 2. AuthHandler delegates to ValidationHandler
    \draw [msg] (4.0,-2.0) -- node[above, font=\xsf] {super.handle(request)} (8.0,-2.0);
    \node[anchor=west, font=\tiny, color=blue!80!black] at (8.1, -2.4) {[Validation Passed]};
    
    % 3. Self call for Core Logic
    \draw [msg] (8.0,-3.1) -- ++(1.4,0) -- ++(0,-0.6) -- node[below, pos=0.85, font=\tiny] {Execute Business Logic} (8.0,-3.7);

    % 4. Return calls
    \draw [ret] (8.0,-4.3) -- (4.0,-4.3);
    \draw [ret] (4.0,-4.7) -- (0,-4.7);

    \end{tikzpicture}
    \caption{UML Sequence Diagram showing successful request execution flow}
...
\begin{itemize}
    \item \textbf{Single Responsibility Principle (SRP):} Each handler encapsulates a single processing concern, leading to clearer separation of logic and improved maintainability.
    
    \item \textbf{Open/Closed Principle (OCP):} New processing steps can be introduced by extending the handler hierarchy without modifying existing, stable components.
